...
select count(*) from salaries where substr(to_date,1,4)='9999';

-- 문제5.
-- 부서는 총 몇 개가 있나요?
	select count(distinct(dept_no)) as 부서개수 from dept_emp;

-- 문제6.  **
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
	select count(*) from dept_manager where substr(to_date,1,4)='9999';
...
-- 현재 급여가 120,000이상 받는 사원은 몇 명이나 있습니까?
	select count(distinct(emp_no)) from salaries where salary >= 120000 and substr(to_date,1,4)='9999'; 
...
-- 현재 Enginner 직책의 사원은 총 몇 명입니까?
	select count(*) from titles where title='Engineer' and substr(to_date,1,4)='9999';
    
-- 문제11 **
-- 사번이 13250(Zeydy)인 지원이 직책 변경 상황을 시간순으로 출력해보세요.
select * from employees inner join titles on titles.emp_no=employees.emp_no where titles.emp_no='13250' order by to_date;